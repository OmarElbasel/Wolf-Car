...
function TableFooter({ className, ...props }: React.ComponentProps<"tfoot">) {
  return (
    <tfoot
      ...
      className={cn(
        "border-t bg-sand/60 font-medium [&>tr]:last:border-b-0",
        className
      )}
      {...props}
    />
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b transition-colors hover:bg-sand/60 has-aria-expanded:bg-sand/60 data-[state=selected]:bg-sand",
        className
      )}
      {...props}
    />
